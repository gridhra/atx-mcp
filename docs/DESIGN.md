...
#### 実写素材でのローカル検証(2026-09-15。素材はリポジトリ外、コミットしない)

Wikimedia Commons の明示ライセンス(PD / CC0 / CC BY / CC BY-SA)画像 7 点(日本の出入国カードのスキャン、
駅名標 2、飲食店メニュー 2、ホワイトボード 2)と Noto Sans JP(OFL)を、リポジトリ外のディレクトリに置いて
release ビルドの stdio 経由で回した。著作権法 30 条の 4 第 1 号(技術の試験の用)に基づくローカル限定の利用で、
画像・処理結果は docs にも Issue にも載せない。

| 観点 | 結果 |
|---|---|
| `inspect_image` | 7 点中 5 点が `has_gps: true`。実写ではこの警告が既定で出ることの価値を再確認 |
| `detect_document` | 6 点で正直に `no_quad_found`(看板・メニュー・掲示は「用紙」ではないので正しい)。**1 点(付箋の多いホワイトボード)で退化した四角形を confidence 0.414 で返し、perspective が画像を引き裂いた**(下記の不具合) |
| `ocr_document` プリセット | スキャン(出入国カード)は 1568 プレビューで罫線・小活字まで判読可。メニュー写真・ホワイトボード写真もグレースケール + 自動レベルで判読性が上がる |
| `detect_text_blocks` | 全点でブロックを返す(9〜24 個)。行高の 1568 換算は 5〜22px。**横に広いメニュー写真(4080×3072)では行高 5px と判定しつつ帯は 1 本**で、エージェントに次の一手が無い(下記の改善) |
| CJK 描画 | Noto Sans JP(可変フォント、9.6MB)を `import_asset` → `font_revision_ids` で参照し、日本語ラベルが警告なしで描画。同一 SVG 内の英数字は同梱 Roboto にフォールバック |

**見つかった不具合と改善(v0.6.x で対応)**

1. `detect_document` の退化四角形: 返った quad は `[[166,4261],[3442,1615],[3303,4383],[3294,4486]]`(br と bl が 100px、
   「左辺」が対角線)。形状の健全性検査(辺長比、内角、対辺比、頂点順序)が無く、confidence の直角性減点も弱かった。
   健全性検査を追加し `min_confidence` を 0.4 → 0.5 に上げる
2. `recommended_bands` は縦方向の帯しか出せず、横幅が律速の画像では 1 本になる。ブロック単位の切り出し
   (`strategy: "blocks"`)を追加し、縮小後に読める大きさになる領域を読み順で返す

**追加 4 点(レシート、手書き掲示、明治期の縦書き新聞、1905 年の 3 段組新聞)**

| 素材 | 結果 |
|---|---|
| レシート(TSE 情報部分、922×790) | 7 ブロック、text_like_area 0.72、行高 1568 換算 41px。1568 プレビューで署名文字列の英数字まで判読可 |
| 手書きの日本語掲示(3264×2448) | 行高 1568 換算 7px の判定で帯 2 本。帯 1 本目は手書き文字が読める大きさになった(帯分割が実写で機能した初例) |
| 縦書き新聞(朝日新聞 1879、1600×2281) | **0 ブロック、`no_text_like_regions`**。文字で埋まった面に対して「文字なし」は誤解を招く。縦書き非対応であることを警告で明示すべき(下記 3) |
| 3 段組の新聞(1905、2240×3255) | 判読は可能だが **2 ブロックしか返らず段組を分けられない**。水平 smearing が段間の空きを越えて結合したか、成分高さの上限(画像高さの 10%)で段の塊が落ちている。多段組は縦方向の投影プロファイルで段境界を先に切るのが筋(下記 4) |

**修正後の再確認(同日)**: 1 の健全性検査(辺長比 ≥ 0.2、内角 90°±45°、対辺比 0.25..4、頂点順序、
`SUPPORT_WEIGHT` 0.7 → 0.5、`min_confidence` 0.4 → 0.5)後、ホワイトボード写真は `no_quad_found` を返し、
合成書類の精度テストは据え置きで通った。2 の `strategy: "blocks"` はメニュー写真で切り出し 3 本になったが、
**1 本目は被写界深度の浅いボケた周辺部**で、ボケた画の断片(高さ 13px)を文字行と誤認していた(実際の文字は 100px 超)。
ピンぼけ・低コントラストの写真では Otsu 後の断片が行高の中央値を引き下げる、という限界。対策候補は
「画像高さの 0.5% 未満の成分をノイズとして捨てる」「行高の四分位範囲が広すぎる場合は警告」。今回は記録のみ(5)。

3. 縦書きの検出(改善 → **推定は撤回、事実ベースの警告に置換**): 当初「縦長で細い成分が多い」で縦書きを推定する
   警告を実装したが、実写で 2 度外れた。(a) smearing 前の成分で数える版は、縦書きの字が正方形の塊なので鳴らない。
   (b) 縦方向 smearing 後の成分で数える版は、横書きの露語 3 段組新聞に「縦書きの疑い(1357 対 289)」の**偽陽性**を出した
   (密な文字面では縦 smearing が行同士を繋ぐ)。合成テストは両版とも通っていたので、実写でしか見つからない失敗だった。
   最終形は推定をやめ、「ブロック 0 件 かつ インク画素比 ≥ 5% かつ 連結成分 ≥ 200」のときだけ
   `dense ink but no horizontal text lines were found; ... vertical ... not supported - rotate 90 degrees ... or read the page directly`
   と事実と回避策だけを返す(ブロックが 1 つでも取れた画像では出さない)。
   **実写で確認**: 縦書き新聞で警告が出て、露語 3 段組新聞では何も出ない(偽陽性なし)
