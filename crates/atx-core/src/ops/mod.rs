...
//! | 作業空間 | op | 理由 |
//! |---|---|---|
//! | **線形光** | `resize` / `rotate` / `perspective` / `crop` / `pad` / `blur` / `median` / `unsharp_mask` / `convolve` / `white_balance` | 画素の**混合**(加重平均・畳み込み)と**露出のスケール**は、物理的な光量に対して行ってはじめて正しい。符号値のまま平均すると暗部に寄る(古典的なガンマ・ブラー誤差) |
