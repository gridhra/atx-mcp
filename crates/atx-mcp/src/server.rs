...
Layered recipes: a recipe may carry {"layers": [...]} instead of (or alongside) a flat operations list. Layers composite bottom-to-top: each layer is {"source": "base" | {"revision_id": "rev_..."}, "ops": [...], "mask"?, "blend_mode"? (one of the 12 W3C separable modes, default "normal"), "opacity"? (0..1, default 1.0)}, its own ops run against its own source, and the result blends onto the running composite. Every layer's source must match the base image's dimensions exactly. When layers are present, the top-level operations becomes the FINISHING pass applied once to the composited result - resize and the final encode belong there, and encode must still be last and appear at most once. Call explain_operation {"operation":"layers"} for the full reference. Example:
{"layers": [
  {"source": "base", "ops": []},
